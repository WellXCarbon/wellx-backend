...
function requireApiKey(req, res, next) {
  ...
  if (given !== DEMO_API_KEY) return res.status(401).json({ ok: false, error: "invalid api key" });
  next();
}
...
app.get("/api/health", (req, res) => res.json({ ok: true, service: "wellx-mvp-v1.4", env: process.env.NODE_ENV || "development" }));
...
app.get("/api/rules/:version", requireApiKey, requireAuth, async (req, res) => {
  try {
    ...
    if (!rs) return res.status(404).json({ ok: false, error: "rule set not found" });
...
app.post("/api/rules", requireApiKey, requireAuth, requireRole("admin", "editor"), async (req, res) => {
  try {
    ...
    if (!version || !Array.isArray(rules) || !rules.length) {
      return res.status(400).json({ ok: false, error: "version and non-empty rules array required" });
    }
    const created = await createRuleSetDraft({ version, rules, created_by: req.user.email, notes: notes || "" });
...
app.post("/api/rules/:version/reject", requireApiKey, requireAuth, requireRole("approver"), async (req, res) => {
  try {
    const rejected = await rejectRuleSet(req.params.version, req.user.email, (req.body && req.body.reason) || "");
...
app.post("/api/evaluate", requireApiKey, requireAuth, async (req, res) => {
  try {
    ...
    const ruleSet = req.body.rule_set_version ? await getRuleSetByVersion(req.body.rule_set_version) : await getActiveRuleSet();
...
app.post("/api/process", requireApiKey, async (req, res) => {
  try {
    ...
    await run("INSERT INTO events (record_id, event_type, message, created_at_utc) VALUES (?, ?, ?, ?)", [recordId, "dataset_uploaded", "Dataset uploaded and hashed", utcNow()]);
    ...
    await run(
      ...
      [recordId, JSON.stringify(evaluation.state), JSON.stringify(evaluation.rule_results), JSON.stringify(evaluation.evaluation_trace), evaluation.final_outcome, evaluation.decision_hash, evaluation.rule_set_version, utcNow()]
    );
    await run("INSERT INTO events (record_id, event_type, message, created_at_utc) VALUES (?, ?, ?, ?)", [recordId, "rules_evaluated", `Outcome: ${evaluation.final_outcome}`, utcNow()]);

    const xrplPayload = { decision_hash: evaluation.decision_hash, rule_version: evaluation.rule_set_version, outcome: evaluation.final_outcome };
    const xrpl = await anchorToXRPL(xrplPayload);
    const finalStatus = evaluation.status === "RULES_PASSED" ? "ANCHORED_ELIGIBLE" : "ANCHORED_REJECTED";

    await run("UPDATE records SET xrpl_tx_hash = ?, xrpl_explorer_url = ?, status = ? WHERE id = ?", [xrpl.txHash, xrpl.explorerUrl, finalStatus, recordId]);
    await run("INSERT INTO events (record_id, event_type, message, created_at_utc) VALUES (?, ?, ?, ?)", [recordId, "xrpl_anchored", `Anchored on XRPL: ${xrpl.txHash}`, utcNow()]);

    res.json({
      ok: true, record_id: recordId, well_id: wellId, sha256_hash: hash,
      rule_set_version: evaluation.rule_set_version,
      state: evaluation.state, rule_results: evaluation.rule_results, evaluation_trace: evaluation.evaluation_trace,
      final_outcome: evaluation.final_outcome, decision_hash: evaluation.decision_hash,
      xrpl_tx_hash: xrpl.txHash, xrpl_explorer_url: xrpl.explorerUrl, wallet_address: xrpl.walletAddress
    });
...
app.post("/api/verify", requireApiKey, requireAuth, async (req, res) => {
  try {
    ...
    const evalRow = await get("SELECT * FROM rule_evaluations WHERE record_id = ? ORDER BY id DESC LIMIT 1", [record_id]);
    const recomputedHash = sha256(payload);
    const valid = recomputedHash === record.sha256_hash;
    res.json({
      ok: true, record_id, valid, stored_hash: record.sha256_hash, recomputed_hash: recomputedHash,
      ...
      xrpl_tx_hash: record.xrpl_tx_hash, xrpl_explorer_url: record.xrpl_explorer_url
    });
  } catch (err) { res.status(500).json({ ok: false, error: err.message }); }
});
...
app.get("/api/records", requireApiKey, requireAuth, async (req, res) => {
  try {
    const rows = await all("SELECT r.*, e.final_outcome, e.decision_hash, e.rule_set_version FROM records r LEFT JOIN rule_evaluations e ON e.record_id = r.id ORDER BY r.id DESC LIMIT 100");
...
app.get("/api/records/:id", requireApiKey, requireAuth, async (req, res) => {
  try {
    ...
    if (!record) return res.status(404).json({ ok: false, error: "record not found" });
    const events = await all("SELECT * FROM events WHERE record_id = ? ORDER BY id ASC", [req.params.id]);
    const evaluation = await get("SELECT * FROM rule_evaluations WHERE record_id = ? ORDER BY id DESC LIMIT 1", [req.params.id]);
...
app.post("/api/admin/users", requireAuth, requireRole("approver"), async (req,res)=>{
 try{
  ...
  await run("INSERT INTO users(email,password_hash,role,created_at_utc) VALUES (?,?,?,?)",
    [email,hash,role,new Date().toISOString()]);
  await logAudit({actor:req.user.email,action:"create_user",entity:"user",entity_id:email});
  res.json({ok:true});
